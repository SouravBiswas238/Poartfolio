...
const Navigation: React.FC = () => {
  ...
  return (
    <nav className={`fixed top-0 left-0 right-0 z-30 transition-all duration-300 ${
      isScrolled ? 'bg-background/90 backdrop-blur-md shadow-lg border-b border-surface/20' : 'bg-transparent'
    }`}>
        ...
        <div className="flex justify-between items-center py-3 sm:py-4">
          ...
          <div className="flex items-center space-x-2 sm:space-x-3">
            <div className="relative group cursor-pointer">
              ...
              >
                <span className="text-white font-bold text-xs sm:text-sm lg:text-lg tracking-wider">SKB</span>
              </div>
              <div 
          ...
          <div className="hidden lg:flex items-center space-x-6 xl:space-x-8">
            ...
            <button
              ...
              className="p-2 xl:p-3 rounded-full bg-surface/40 backdrop-blur-sm transition-all duration-300 hover:scale-110 flex items-center justify-center"
              aria-label="Change theme"
            >
              <Settings size={18} className="xl:w-5 xl:h-5 text-primary animate-spin-slow" />
            </button>
          </div>
          ...
          <div className="lg:hidden flex items-center space-x-2 sm:space-x-3">
            ...
            <button
              ...
              aria-label="Change theme"
            >
              <Settings size={16} className="sm:w-[18px] sm:h-[18px] text-primary animate-spin-slow" />
            </button>
            ...
            <div className="p-3 sm:p-4">
              {navItems.map((item, index) => (
                <button
                  ...
                  } ${index !== navItems.length - 1 ? 'border-b border-surface/30' : ''}`}
                >
                  {item.label}
                </button>
              ))}
          ...
          <div className="relative bg-surface/95 backdrop-blur-md rounded-xl sm:rounded-2xl border border-primary/20 w-full max-w-xs sm:max-w-md max-h-[80vh] overflow-y-auto">
            <div className="p-4 sm:p-6">
              ...
              <div className="flex items-center justify-between mb-4 sm:mb-6">
                <h3 className="text-base sm:text-lg lg:text-xl font-bold text-text">Choose Theme</h3>
                <button
                  onClick={() => setShowThemeModal(false)}
                  className="p-1.5 sm:p-2 hover:bg-primary/10 rounded-lg transition-colors duration-200"
                >
                    ...
                    <div className="flex items-center justify-between">
                      <div className="flex items-center space-x-2 sm:space-x-3">
                        ...
                        <div className="text-left">
                          <p className="font-medium text-text text-xs sm:text-sm">{theme.name}</p>
